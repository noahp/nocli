...
int test_nocli_prompt(void){
    ...
    // check result
    if(mock_output_buffer_idx != sizeof(NOCLI_CONFIG_ENDLINE_STRING PROMPT_1_STRING) - 1){
        ERROR_EXIT;
    }
    if(memcmp(mock_output_buffer, NOCLI_CONFIG_ENDLINE_STRING PROMPT_1_STRING, mock_output_buffer_idx) != 0){
    ...
    if(mock_output_buffer_idx != sizeof(NOCLI_CONFIG_ENDLINE_STRING PROMPT_2_STRING) - 1){
        ERROR_EXIT;
    }
    if(memcmp(mock_output_buffer, NOCLI_CONFIG_ENDLINE_STRING PROMPT_2_STRING, mock_output_buffer_idx) != 0){
        printf("%.*s\n", (int)mock_output_buffer_idx, mock_output_buffer);
        ERROR_EXIT;
    }
...
static int test_command_call(void){
    ...
    #define BAD_COMMAND_RESPONSE_STRING NOCLI_CONFIG_ENDLINE_STRING "nocli $"\
         BAD_COMMAND_STRING NOCLI_CONFIG_ENDLINE_STRING "error, command not found"\
         NOCLI_CONFIG_ENDLINE_STRING "nocli $"
    if(Nocli_Feed(&nocli_ctx, BAD_COMMAND_STRING "\n", sizeof(BAD_COMMAND_STRING "\n") - 1)){
        ERROR_EXIT;
    }
    ...
    #define GOOD_COMMAND_RESPONSE_STRING "function1 arg1" NOCLI_CONFIG_ENDLINE_STRING "nocli $"
    mock_output_buffer_idx = 0;
    if(Nocli_Feed(&nocli_ctx, GOOD_COMMAND_STRING, sizeof(GOOD_COMMAND_STRING) - 1)){
        ERROR_EXIT;
    }
    if(mock_output_buffer_idx != sizeof(GOOD_COMMAND_RESPONSE_STRING) - 1){
        printf(">> %d %d\n", mock_output_buffer_idx, sizeof(GOOD_COMMAND_RESPONSE_STRING));
        printf(">> %.*s\n", mock_output_buffer_idx, mock_output_buffer);
        ERROR_EXIT;
    }
    if(memcmp(mock_output_buffer, GOOD_COMMAND_RESPONSE_STRING, mock_output_buffer_idx) != 0){
        ERROR_EXIT;
    }
...
static int test_help(void){
    ...
    #define HELP_RESULT_STRING NOCLI_CONFIG_ENDLINE_STRING "nocli $?\b?"\
         NOCLI_CONFIG_ENDLINE_STRING\
         "?" NOCLI_CONFIG_ENDLINE_STRING\
         "help" NOCLI_CONFIG_ENDLINE_STRING\
         "function1" NOCLI_CONFIG_ENDLINE_STRING\
         "nocli $"
    ...
    if(memcmp(mock_output_buffer, HELP_RESULT_STRING, mock_output_buffer_idx) != 0){
        printf("%.*s\n", (int)mock_output_buffer_idx, mock_output_buffer);
        ERROR_EXIT;
    }
